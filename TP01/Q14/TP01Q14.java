class TP01Q14 {
    ...
    public static Boolean parser(char[] exp){
        return parser(exp, 0, 0);
    }

    public static Boolean parser(char[] exp, int i, int j){
        boolean res = false;

        // interpreta expressao recursivamente e realiza operacoes em sua devida ordem
        if(i < exp.length){
            ...
            if( (exp[i] == ')') && (j > 0) ){
                ...
                res = parser(exp, i, j-1);
                
            }

            res = parser(exp, i+1, i);
        }
